...
public class NaquadriaOre extends NaquadriaExplosive implements IDisintegrable{
	...
	public void updateTick(World w, int x, int y, int z, Random r){
		...
		if(r.nextBoolean() && w.getBlockId(x+1, y, z) == naquadah)
			w.setBlockAndMetadataWithNotify(x+1, y, z, this.blockID, 0, Helper.SETBLOCK_NO_UPDATE);
		if(r.nextBoolean() && w.getBlockId(x-1, y, z) == naquadah)
			w.setBlockAndMetadataWithNotify(x-1, y, z, this.blockID, 0, Helper.SETBLOCK_NO_UPDATE);
		if(r.nextBoolean() && w.getBlockId(x, y+1, z) == naquadah)
			w.setBlockAndMetadataWithNotify(x, y+1, z, this.blockID, 0, Helper.SETBLOCK_NO_UPDATE);
		if(r.nextBoolean() && w.getBlockId(x, y-1, z) == naquadah)
			w.setBlockAndMetadataWithNotify(x, y-1, z, this.blockID, 0, Helper.SETBLOCK_NO_UPDATE);
		if(r.nextBoolean() && w.getBlockId(x, y, z+1) == naquadah)
			w.setBlockAndMetadataWithNotify(x, y, z+1, this.blockID, 0, Helper.SETBLOCK_NO_UPDATE);
		if(r.nextBoolean() && w.getBlockId(x, y, z-1) == naquadah)
			w.setBlockAndMetadataWithNotify(x, y, z-1, this.blockID, 0, Helper.SETBLOCK_NO_UPDATE);
	}
